...
| `./alv-log-formats.txt` → `/app/alv-log-formats.txt` | 利用者定義のログ書式。起動スクリプトが空ファイルを用意する（無いと Docker が同名のディレクトリを作り、以後「通常ファイルではありません」で失敗し続けるため） |
...
組み込み書式に無い並びのログは、正規表現で書式を自分で定義すると読めるようになります。
**中身の管理は利用者の責任**です。
...
|---|---|---|
| `ts` | **必須** | 日時。日付まで含めること |
| `client` | **必須** | アクセス元。クライアント IP 欄の絞り込みに使う |
| `status` | **必須** | ステータス。`-` なら「記録なし」 |
| `method` | 任意 | 省くと `-` |
| `path` | 任意 | 省くと `-` |
| `host` | 任意 | リクエストを受けたホスト。省くと `client` と同じ値 |
| `xff` | 任意 | X-Forwarded-For。省くと空 |
...
そこを読む必要が無ければ `.*$` でまとめて受ければ十分です。

**タイムゾーン**: 日時書式にオフセット（`Z` や `XXX`）があればそれを使い、無ければ
**UTC とみなします**。組み込み書式がオフセットの無いログを `+0000` として扱うのと同じで、
...
- 書式は自動判定の候補にも入ります。**組み込みの選び方には手を入れていません**
  ...
  利用者定義がそれより多く当たったときだけ入れ替わります。同点なら組み込みです。
...
（同じ測定で 1,854ms → 1,857ms。1,773〜2,032ms のばらつきの中なので差は無い）。
